mlr_cvglmnet <- function(training_data,
  ...
  lrn_enet <- lrn("regr.cv_glmnet", alpha = alpha, s = "lambda.min")
  lrn_enet$train(enet)
  return(lrn_enet)
}
...
test_cvglmnet <- function(training_data,
  ...
  mlr_enet_predict <- mlr_enet$predict_newdata(testing_data)

  x_train <- as.matrix(training_data[, !..y])
  y_train <- training_data[[y]]

  glm_enet <- cv.glmnet(x_train, y_train, alpha = alpha, family = "gaussian")
