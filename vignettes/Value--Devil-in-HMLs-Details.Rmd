...
In the construction of the original "International sample" (or "Global ex US"), \textcite{asness-frazzini-2013} included the following countries: Austria, Australia, Belgium, Canada, Switzerland, Germany, Denmark, Spain, Finland, France, Italy, Japan, Netherlands, Norway, New Zeland, Portugal, Singapore, Sweden, United Kingdom, and United States of America. Latest data provided by AQR, extends this list of countries including also Ireland, Israel, and Portugal. We study the full panel of countries in the newest International sample, but restricting attention to the original sample would certainly be possible.
```{r Get Factors Data}
## Load libraries
library(xts)
library(plm)
...
# Indexes to determine panels structure
# 'DATE.ID', each date gets a numeric id 
periods <- unique(factors.data$DATE)
periods.id <- 1:length(periods)
periods.id <- data.frame('DATE'=periods, 'DATE.ID'=periods.id)
factors.data <- merge(factors.data, periods.id, by='DATE')
periods.intl <- unique(factors.data.intl$DATE)
periods.intl.id <- 1:length(periods.intl)
periods.intl.id <- data.frame('DATE'=periods.intl, 'DATE.ID'=periods.intl.id)
factors.data.intl <- merge(factors.data.intl, periods.intl.id, by='DATE')

### Time-series regressions US ###
# HML annual & lagged
ts.reg.us.al <- plm::plm(
  HML ~ MKT.RF + SMB + UMD + STR + HML.DEV, 
  data=factors.data[which(factors.data$DATE %in% dates.us), ],
  model='pooling', index='DATE.ID'
)
summary(ts.reg.us.al)
betas.us.al <- coef(ts.reg.us.al)
names(betas.us.al) <- paste('BETA', names(betas.us.al), sep='.')
betas.us.al

# HML monthly & current (HML devil)
ts.reg.us.mc <- plm::plm(
  HML.DEV ~ MKT.RF + SMB + UMD + STR + HML, 
  data=factors.data[which(factors.data$DATE %in% dates.us), ],
  model='pooling', index='DATE.ID'
)
summary(ts.reg.us.mc)
betas.us.mc <- coef(ts.reg.us.mc)
names(betas.us.mc) <- paste('BETA', names(betas.us.mc), sep='.')
betas.us.mc

### Time-series regressions INTL ###
# HML annual & lagged
ts.reg.intl.al <- plm::plm(
  HML.INTL ~ MKT.RF.INTL + SMB.INTL + UMD.INTL + HML.DEV.INTL, 
  data=factors.data.intl[which(factors.data.intl$DATE %in% dates.intl), ],
  model='pooling', index='DATE.ID'
)
summary(ts.reg.intl.al)
betas.intl.al <- coef(ts.reg.intl.al)
names(betas.intl.al) <- paste('BETA', names(betas.intl.al), sep='.')
betas.intl.al

# HML monthly & current (HML devil)
ts.reg.intl.mc <- plm::plm(
  HML.DEV.INTL ~ MKT.RF.INTL + SMB.INTL + UMD.INTL + HML.INTL, 
  data=factors.data.intl[which(factors.data.intl$DATE %in% dates.intl), ],
  model='pooling', index='DATE.ID'
)
summary(ts.reg.intl.mc)
betas.intl.mc <- coef(ts.reg.intl.mc)
names(betas.intl.mc) <- paste('BETA', names(betas.intl.mc), sep='.')
betas.intl.mc
```
