...
dates.us <- as.Date(Reduce(intersect, list(factors.data$DATE, US.SAMPLE.DATE.PAPER))) # assets.data$DATE
# Merge
...
names(betas.us.al) <- paste(
  'BETA', c('(Intercept)', attr(ts.reg.us.al$terms, 'term.labels')), sep='.'
)
betas.us.al
...
names(betas.us.mc) <- paste(
  'BETA', c('(Intercept)', attr(ts.reg.us.mc$terms, 'term.labels')), sep='.'
)
betas.us.mc
```
