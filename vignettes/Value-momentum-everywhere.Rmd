...
## Import AQR data
# VME data
source(file.path('inst', 'parsers', 'VME-Factors-orig.R'))
...
VME_table1 <- function(VME.Portfolios, geometric = FALSE) {

return <- PerformanceAnalytics::Return.annualized(VME.Portfolios, geometric = FALSE)
SD <- PerformanceAnalytics::sd.annualized(VME.Portfolios, geometric = FALSE)
sharpe <- PerformanceAnalytics::SharpeRatio.annualized(VME.Portfolios, geometric = FALSE)
...
source("~/R/expected-returns/inst/parsers/FFdownloads_factors_package.R")
...
days.diff <- diff(seq.Date(min.tp, max.tp, by='month'))
...
# Factors data
index(MSCI.WI) <- as.yearmon(index(MSCI.WI))
...
index(CRP) <- as.yearmon(index(CRP))
...
## attemptingg to reproduce regression below using fitTsfm instead

ts.reg2 <- fitTsfm(asset.names = colnames(portfolios2[,(11:13)]),
                   factor.names = colnames(portfolios2[,c("VAL.EVR", "MOM.EVR", "MSCI.RF")]),
                   data = portfolios2)
...
ts.reg <- plm::pmg(
  ...
  data=portfoliosDT, index=c('PORTF.ID', 'DATE.ID')
)
## Estimates
