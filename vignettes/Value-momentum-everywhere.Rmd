...
# MSCI 
# portfolios$MSCI.RET <- zoo::na.locf(portfolios$RET)  #This can't be right***
portfolios$MSCI.RET <- portfolios$RET
...
portfolios <- portfolios[ff.dates,c(colnames(CRP), 'MSCI.RF','RET', colnames(FFC4))]
...
# portfoliosDT <- portfoliosDT[order(portfoliosDT[, 'PORTF.ID'], portfoliosDT[, 'DATE.ID']), ]
 portfoliosDT[order(PORTF.ID, DATE.ID)] #I have no idea what the point of this is
portfoliosDT <- portfoliosDT[order(PORTFNAME,Date)]
