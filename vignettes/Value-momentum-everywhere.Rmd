...
## Merge FF4 and MSCI
FF4 <- merge(FF3[ff.dates, ], MOM[ff.dates, ])
data <- merge(MSCI.WI, FF4)
# Linear interpolation of interior MSCI discrete returns,
# accounting for publication dates mismatches
data$MSCI.RET <- na.fill(data$RET, c(NA, 'extend', 'extend'))
data$MSCI.RF <- data$MSCI.RET - data$RF
data <- data.frame(
  DATE=ff.dates,
  data[ff.dates, c('MSCI.RF', colnames(FF4))],
  row.names=NULL
)
...
## TABLE 6, Panel A
## US
us.factors.vars <- c('MKT.RF', 'SMB', 'HML', 'MOM') 
fv.seqs <- 1:length(us.factors.vars)
...
## Global
global.factors.vars <- c('MSCI.RF', 'VAL.EVR', 'MOM.EVR')
...
for (j in rev(fv.seq)) {
  k <- setdiff(1:comb.idxs[j], 1:comb.idxs[j+1])
...
# Estimates
models.summaries <- lapply(ts.reg.global, summary)
lapply(1:kk, function(x) models.summaries[[x]]$r.squared)
```
