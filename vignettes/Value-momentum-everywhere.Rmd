...
portfolios <- merge(portfolios, dates.id, by='DATE')
# 'PORTF.ID', each portfolio gets unique numeric id
portf <- unique(portfolios$PORTF)
portf.id <- 1:length(portf)
portf.id <- data.frame('PORTF'=portf, 'PORTF.ID'=portf.id)
portfolios <- merge(portfolios, portf.id, by='PORTF')
portfolios <- portfolios[order(portfolios[, 'PORTF.ID'], portfolios[, 'DATE.ID']), ]
row.names(portfolios) <- NULL
