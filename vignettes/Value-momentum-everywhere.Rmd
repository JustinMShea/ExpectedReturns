...
# Refactor try by Jiarui
test_using_origin_script<-plm::plm(
    TSMOM.FX ~ MSCI.RF + CM.MARKET + FI.MARKET + SMB + HML + MOM, data=data,
    model='pooling', index='DATE.ID'
)
coefficients(test_using_origin_script)

#refactored
data.test = data
row.names(data.test) <-data.test$DATE
data.test$DATE <- NULL
test_using_new_script<-fitTsfm(asset.names=c('MSCI.RF', 'CM.MARKET', 'FI.MARKET', 'SMB', 'HML', 'MOM'), factor.names=c('TSMOM.FX'), data = data.test)

test_using_new_script.coefs <- cbind(test_using_new_script$alpha, test_using_new_script$beta)
test_using_new_script.coefs
```
