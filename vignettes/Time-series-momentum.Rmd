...
source(parser.path)
# TODO: 'GSCI' is currently missing
# Bonds
# TODO: 'CORP.XS' inappropriate substitute of 'BOND'
parser.path <- file.path('inst', 'parsers', 'CRP.R')
source(parser.path)
...
# MSCI interior discrete returns are linearly interpolated to account for
# publication dates mismatches with respect to FF data dates and AQR's data sets
data$MSCI.RET <- na.fill(data$RET, c(NA, 'extend', 'extend'))
data$MSCI.RF <- data$MSCI.RET - data$RF
# Likewise bonds factors data
data <- merge(CRP, data)
...
y <- colnames(TSM[, -1])
# TODO: 'CORP.XS' inappropriate substitute of 'BOND'
X <- c('MSCI.RF', 'CORP.XS', 'SMB', 'HML', 'RF', 'MOM')
