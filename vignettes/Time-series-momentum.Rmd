...
# Bonds factor
index(FI.MARKET) <- as.yearmon(index(FI.MARKET))
data <- merge(data, FI.MARKET)
# Commodities Market
index(CM.MARKET) <- as.yearmon(index(CM.MARKET))
data <- merge(data, CM.MARKET)
# VME factors
data <- merge(data, VME.FACTORS)
# VIX Index
index(VIX.RET) <- as.yearmon(index(VIX.RET))
data <- merge(data, VIX.RET)
# TED Spread
index(TED.SPREAD) <- as.yearmon(index(TED.SPREAD))
data <- merge(data, TED.SPREAD)
# TSMOM(12, 1) strategy returns
index(TSM) <- as.yearmon(index(TSM))
data <- merge(data, TSM)
...
index(TED20) <- as.yearmon(index(TED20))
index(VIX20) <- as.yearmon(index(VIX20))


data <- merge(data, TED20, VIX20)

ff.dates <- as.yearmon(ff.dates)
...
data <- data[-((which(data$DATE=="2009-12-01")+1):nrow(data)),]
...
row.names(mom.data.test) <-as.Date(mom.data$DATE) 
