...
Next, following authors we run multivariate regressions of the returns of comodities indexes portfolios, equal-weighted and long-short, on investment styles. These are namely momentum, measured by portofolios previous 12-month return, value measured as the negative 48-month return 12-months ago (i.e., long-term reversal), and carry measured as the backwardated/contango value of the commodity index. 

```{r Table 7 NBER paper}
# Value
# measured as the negative 48-month return 12-months ago (i.e., long-term reversal)
...
# Momentum
# measured by previous 12-month return
...
colnames(data.styles) <- c(
  ...
  , 'VAL.EW', 'VAL.LS', 'MOM.EW', 'MOM.LS'
)

# Regressions on Equal-weighted and Long-short portfolios
## Monthly
mods <- list(
  EW = c('XRET.EW', 'MOM.EW', 'VAL.EW', 'PFC.AGG')
  , LS = c('XRET.LS', 'MOM.LS', 'VAL.LS', 'PFC.AGG') 
)
style.regs <- lapply(mods, function(x) {
  model.formula <- formula(
    paste(
      x[1], paste(x[2:length(x)], collapse='+'),
      sep='~' 
    )
  )
  lm(
    model.formula, data=data.styles
  )
})
lapply(style.regs, summary)
```
