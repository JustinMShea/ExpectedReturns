...
plot(
  ...
  main='Excess Spot Return/Interest Rate–Adjusted Carry Return Decomposition \n(cumulative returns %)',
  screens=factor(1, 1, 1), col=c('black', 'blue', 'red'),
  legend.loc='topleft', lty=c(1, 2, 2), lwd=rep(0.95, 3)
)
# Panel B
plot(
  cbind(cumsum(COMLR$XRET.EW), cumsum(COMLR$SRET.EW), cumsum(COMLR$CARRY.EW)) * 100,
  main='Spot/Carry Return Decomposition \n(cumulative returns %)',
  screens=factor(1, 1, 1), col=c('black', 'blue', 'red'),
  legend.loc='topleft', lty=c(1, 2, 2), lwd=rep(0.95, 3)
)
```
