...
LmHorizon <- function(Y, X, horizon, data) {
  ...
  for (h in horizon) {
    ...
    for(y in Y) {
      ...
      names(out[[i]])[] <- paste('H', horizon, sep='')
    }
  }
  names(out) <- Y
  return(out)
