...
TsmomPredictability <- function(X, signal=NULL, estimator, lookback, holding, lags, ...) {
  ...
  # Regressions
  xa.reg <- vector('list', nlags)
  xa.nwts <- vector('numeric', nlags)
  for (l in lags) {
    ...
    # xa.nwts[l] <- lmtest::coeftest(
    #   xa.reg[[l]], vcov.=sandwich::NeweyWest(xa.reg[[l]])
    # )[2, 't value']
  }
  return(
    list(
...
# Lags (in months)
test <- TsmomPredictability(
  futures.data.monthly, 
  # signal=c('SIGN', 'TREND', 'SMT'),
