...
futures.summary <- lapply(futures.data.monthly, function(x) {
  x.ret <- x$Comp.Return
  avg <- mean(x.ret, na.rm=TRUE) * sqrt(261) * 100
  vol <- sd(x.ret, na.rm=TRUE) * sqrt(261) * 100
  sr <- PerformanceAnalytics::SharpeRatio.annualized(x.ret, scale=12)
