...
AggregateByType <- function(X, y, symbols, type) {
  ...
  X <- Reduce(function(...) merge(..., all=TRUE), X)
...
mom.signals.avg <- lapply(mom.signals, function(x) {
  x.inst <- Reduce(function(...) merge(..., all=TRUE), x)
  x.inst.avg <- xts::xts(rowMeans(x.inst, na.rm=TRUE), zoo::index(x.inst))
  return(x.inst.avg)
})
mom.signals.avg <- Reduce(function(...) merge(..., all=TRUE), mom.signals.avg)
colnames(mom.signals.avg) <- signals
cor(mom.signals.avg)
...
ls.positions <- lapply(mom.signals, function(x) {
  x.inst <- Reduce(function(...) merge(..., all=TRUE), x)
  lsp <- apply(x.inst, 2, function(j) {
    long <- length(which(j == 1))
    short <- length(which(j == (-1)))
    los <- rbind(long, short)
