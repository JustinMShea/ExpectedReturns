...
$$
\sigma_{\textrm{YZ}}^{2}(t) = \sigma_{o}^{2}(t, D) + k\sigma_{\textrm{STDEV}}^{2}(t, D) + (1 - k)\sigma_{\textrm{RS}}^{2}(t)
$$
...
EstimateVolatility <- function(X, estimator, ...) {
  vol.res <- vol.bias <- vto <- vector('list', length(estimator))
  vol.est <- lapply(X, function(x) {
    # Get dates, OHLC prices, and convert
    x <- x[, 1:5]
    x <- x[, !vapply(x, is.character, FUN.VALUE=logical(1L))]
    x <- xts(x[, -1], order.by=x[, 1])
    for (m in 1:length(estimator)) {
      vol.res[[m]] <- TTR::volatility(x, calc=estimator[m], ...) 
      vol.bias[[m]] <- 1 / nrow(x) * sum(vol.res[[1]] - vol.res[[m]], na.rm=TRUE)
      vto[[m]] <- 1 / (nrow(x) + 1) * sum(abs((1 / vol.res[[m]]) - (1 / lag(vol.res[[m]]))), na.rm=TRUE)
    }
    names(vol.res) <- names(vol.bias) <- names(vto) <- estimator
    return(list(vol.est=vol.res, vol.bias=vol.bias, VTO=vto))
  })
  return(vol.est)
}
