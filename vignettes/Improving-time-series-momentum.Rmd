...
AvgCor <- function(X, y, symbols, type, ...) {
  ...
  #' @param X A `list` of `xts` objects storing instruments data
  #' @param y A character vector, the names of objects to extract from `X` (e.g., momentum signals, volatility estimators)
...
panels.AB <- lapply(list(avg.bias, avg.abrd), function(x) {
...
```{r Figure 3}
# Volatility estimators average ranks
# across futures contracts
vol.avg.ranks <- sapply(panels.AB, function(x) x[nrow(x), ])
barplot(
  t(vol.avg.ranks), 
